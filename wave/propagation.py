...
class WavePropagator:
    ...
    def propagate(
        ...
        # base_amplitude and base_frequency are empirically tuned to the
        # fold line resonance system. base_amplitude = height * 0.085 gives
        # waveform amplitude in [0.1, 1.0] for typical prompts (height ≈ 10).
        # base_frequency = width / 12.0 was tuned to maximize fold line hit
        # rate — width/13.0 was too sparse, width/12.0 produces the right
        # phase density against the 512-point Fibonacci lattice.
        # Do not "derive" these — they are calibration constants for the
        # fold line resonance system, not geometric identities.
        base_amplitude  = 1.0   # unit amplitude — observer reads the field, does not inject energy
        base_frequency  = width / 12.0   # calibrated to align spin phase with symbol lattice indices
    ...
    def propagate_generative(
        ...
        if not waveform_full:
            ...
            return prop_result
        waveform_np = np.array(
            [x for x in waveform_full if isinstance(x, (int, float))], dtype=float
        )
        ...
        result.update({
            "waveform_sample":  list(out_wave[:8]) + ["..."] if len(out_wave) > 8 else list(out_wave),
            "waveform_full":    out_wave.tolist(),
            "persistence":      round(gen["consensus_pers"], 6),
            "consensus_pers":   round(gen["consensus_pers"], 4),
            "dispersion_peak":  gen.get("dispersion_peak", 0.0),
            "dispersion_high_regions": gen.get("dispersion_high_regions", 0),
            "mode":             "generative",
        })
