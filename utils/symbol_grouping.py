...
# ── Derived group radius ───────────────────────────────────────────────────────
# Mean symbol spacing in the 512-point Fibonacci lattice: 512 / 27 ≈ 18.96
# Scaled by φ: 18.96 × 1.618 ≈ 30.67 → ceiling at 31 (Mersenne prime)
#
# At radius 20 (old): mean spacing ≈ radius → most symbols absorb only one
# neighbour → ~18 singleton-pair groups locked by the greedy sweep ceiling.
#
# At radius 31 (derived): symbols can absorb 2–3 neighbours → groups form
# with meaningful Dual-13 net_signed_value rather than single-letter identity.
# 31 is the Mersenne prime already used as the bipolar lattice node count —
# the same geometric constant recurs because both are solving the same problem
# of incommensurable cycle distribution across a bounded space.
_MEAN_SYMBOL_SPACING = _LATTICE_POINTS / len(_SYMBOLS)   # 512/27 ≈ 18.96
_PHI_GROUP           = (1 + math.sqrt(5)) / 2            # φ ≈ 1.618034

# Group radius — how far from a seed symbol to sweep for neighbors.
# Must be LESS than mean_spacing to keep groups distinct.
# Derived: mean_spacing × 1/φ² ≈ 18.96 × 0.382 ≈ 7.24 → nearest Mersenne prime = 7
# Previous value was mean_spacing × φ ≈ 30.67 which absorbed 3+ neighbors
# per seed, causing over-merging to ~12 groups from 27 symbols.
_GROUP_RADIUS        = 7   # = Mersenne prime nearest to mean_spacing × 1/φ² ≈ 7.24

# Merge pass — after initial greedy sweep, merge groups whose nearest members
# are within this radius. Only merges truly adjacent groups.
# Derived: _GROUP_RADIUS × 1/φ² ≈ 7 × 0.382 ≈ 2.67 → 3
_MERGE_RADIUS        = round(_GROUP_RADIUS * (1.0 / (_PHI_GROUP ** 2)))  # ≈ 3

# Minimum imprint to be considered an active resonance point
_IMPRINT_THRESHOLD = 0.016395102 / 3  # AD/3 ≈ 0.005465 — derived from asymmetric delta

# Odd/even tension asymmetry — now applied to signed integer values
_ODD_SCALE         = 1.0                              # full weight for vertical builders
_EVEN_SCALE        = 2.0 / (1.0 + 5**0.5)            # 1/φ ≈ 0.618 for horizontal recognizers
_MOD3_INTERFERENCE = 1.0 / ((1.0 + 5**0.5) / 2)**2  # 1/φ² ≈ 0.382 — parity threshold
...
class SymbolGroup:
    ...
    @property
    def dual13_gid(self) -> int:
        """
        Canonical Dual-13 group identifier — same coordinate space as
        bipolar lattice waypoint gid values.

        Derived from net_signed_value, clamped to ±13.
        Positive groups (net > 0) → +1..+13 (positive arm, state-2 territory)
        Negative groups (net < 0) → -1..-13 (negative arm, state-1 territory)
        Balanced groups (net = 0) → 0       (boundary/regulator)

        This is the bridge between symbol_grouping and bipolar_lattice:
        a word whose dominant dual13_gid is +N maps to the same Dual-13
        position as bipolar lattice positive arm node with gid=+N.
        State-2 (interference mode) emerges naturally for positive-arm words
        without any explicit ternary routing in the pipeline.
        """
        net = self.net_signed_value
        if net == 0:
            return 0
        return max(-13, min(13, net))

    def to_dict(self) -> Dict[str, Any]:
        return {
            "group_id":          self.group_id,
            "dual13_gid":        self.dual13_gid,
...
class SymbolGrouping:
    ...
    def __init__(self):
        ...
        self.symbol_map:   Dict[str, int]          = {}  # symbol → group_id (sequential)
        self.gid_map:      Dict[str, int]          = {}  # symbol → dual13_gid
        self.lattice_map:  Dict[str, int]          = {}
        self._last_imprint_sum = 0.0
        self._recompute_threshold = 0.02
    ...
    def _compute_groups(self) -> None:
        ...
        Pass 1 — Greedy sweep (same as before but with derived radius):
          Sort imprinted symbols by lattice index. For each unassigned symbol,
          sweep forward and backward within _GROUP_RADIUS and absorb neighbours.
          This forms initial clusters based on direct proximity.

          With _GROUP_RADIUS = 31 (derived: mean_spacing × φ ≈ 30.67, Mersenne
          prime ceiling), each symbol can absorb 2–3 neighbours rather than
          just one, producing groups with meaningful Dual-13 net_signed_value.

        Pass 2 — Merge pass (new):
          After initial clustering, scan all group pairs. If the closest
          lattice indices between two groups are within _MERGE_RADIUS (≈ 12),
          merge them. This allows natural conjunctions — symbols that are
          geometrically adjacent but were separated by the greedy sweep order
          find each other in the merge pass.

          _MERGE_RADIUS = _GROUP_RADIUS × 1/φ² ≈ 12 — tight enough to only
          merge genuinely adjacent groups. Prevents runaway merging.

        The result: groups whose members share genuine geometric proximity in
        the Fibonacci lattice, with Dual-13 net_signed_value reflecting the
        combined geometry of all members rather than a single letter's identity.
        """
        ...
        for sym, lidx, imp in imprinted:
            ...
            # Sweep all other imprinted symbols within _GROUP_RADIUS
            # Both forward and backward — not just forward as in the old version.
            # This prevents sweep-order bias where early symbols claim neighbours
            # that later symbols would have grouped with more naturally.
        ...
        # ── Pass 2: Merge pass ────────────────────────────────────────────────
        # Merge groups whose nearest lattice indices are within _MERGE_RADIUS.
        # Only merges imprinted groups — singletons from unimprinted symbols
        # stay independent (they haven't been touched by resonance yet).
        merged = True
        while merged:
            merged = False
            surviving: List[SymbolGroup] = []
            absorbed:  Set[int]          = set()  # group_ids that were merged in

            for i, g1 in enumerate(new_groups):
                if g1.group_id in absorbed:
                    continue
                # Check all groups after this one
                for j, g2 in enumerate(new_groups):
                    if i >= j:
                        continue
                    if g2.group_id in absorbed:
                        continue
                    # Skip unimprinted singleton groups from merge pass
                    if g1.tension_centroid < _IMPRINT_THRESHOLD:
                        continue
                    if g2.tension_centroid < _IMPRINT_THRESHOLD:
                        continue
                    # Find minimum lattice distance between any member pair
                    min_dist = min(
                        abs(i1 - i2)
                        for i1 in g1.lattice_indices
                        for i2 in g2.lattice_indices
                    )
                    if min_dist <= _MERGE_RADIUS:
                        # Merge g2 into g1
                        for sym, lidx in zip(g2.members, g2.lattice_indices):
                            g1.add(sym, lidx)
                        g1.update_centroid(imprints)
                        absorbed.add(g2.group_id)
                        merged = True

                if g1.group_id not in absorbed:
                    surviving.append(g1)

            new_groups = surviving

        # Renumber group_ids sequentially after merging
        for new_id, grp in enumerate(new_groups):
            grp.group_id = new_id

        self.groups = new_groups

        # Rebuild symbol → group_id map AND symbol → dual13_gid map
        self.symbol_map = {}
        self.gid_map    = {}
        for grp in self.groups:
            for sym in grp.members:
                self.symbol_map[sym] = grp.group_id
                self.gid_map[sym]    = grp.dual13_gid
    ...
    def gid_for(self, symbol: str) -> int:
        """
        Return the Dual-13 gid for a symbol — the canonical coordinate
        shared with bipolar lattice waypoint gid values.

        Returns 0 for unknown symbols or boundary/regulator groups.
        Positive (+1..+13) = positive arm territory (state-2 nodes).
        Negative (-1..-13) = negative arm territory (state-1 nodes).
        """
        self._ensure_groups()
        return self.gid_map.get(symbol, 0)
    ...
    def pair_tension(self, s1: str, s2: str) -> Dict[str, Any]:
        ...
        if (v1 > 0 and v2 > 0) or (v1 < 0 and v2 < 0):
            ...
            if abs(v2) > abs(v1):
                base *= (1.0 + invariants.asymmetric_delta * 6)  # expansive ≈ 1.098
            else:
                base *= (1.0 - invariants.asymmetric_delta * 6)  # contractive ≈ 0.902
        ...
        return {
            ...
            "dual13_gids":   (g1.dual13_gid, g2.dual13_gid),
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        # Count groups by arm territory
        positive_arm = sum(1 for g in self.groups if g.dual13_gid > 0)
        negative_arm = sum(1 for g in self.groups if g.dual13_gid < 0)
        boundary     = sum(1 for g in self.groups if g.dual13_gid == 0)
        return {
            ...
            "mean_sym_spacing":  round(_MEAN_SYMBOL_SPACING, 2),
